(function () {
  ...
  M.api("/v1/sellers/" + encodeURIComponent(id) + "/public").then(function (d) {
    ...
    var listings = (d.listings || []).map(function (l) {
      return '<a class="mk-card" href="' + M.esc(l.publicUrl || M.listingUrl(l.id)) + '"><div class="mk-card-body"><h3>' + M.esc(l.title) + "</h3><p class='mk-card-price'>" + M.euro(l.price) + "</p></div></a>";
