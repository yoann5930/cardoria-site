...
test("all active frontend runtime mirrors are synchronized",()=>{assert.equal(adminRuntime,admin);assert.equal(embeddedAdminRuntime,embeddedAdmin);assert.equal(publisherRuntime,publisher);assert.equal(sellerRuntime,seller);assert.equal(ownerActionsRuntime,ownerActions);assert.equal(viewerActionsRuntime,viewerActions);assert.equal(runtimeLive,rootLive);assert.equal(runtimeAdmin,rootAdmin);assert.equal(runtimeSeller,rootSeller);});
