  ...
  function renderOrders(orders) {
    A.qs("#mkOrdersBody").innerHTML = (orders || []).map(function (o) {
      ...
      return "<tr><td>" + esc(o.id) + "</td><td>" + esc(o.listingTitle) + "</td><td>" + esc(o.buyerEmail) + "</td><td>" + A.euro(o.total) + "</td><td>" + esc(o.status) + " / " + esc(o.paymentStatus || "—") + "</td><td>" +
        '<span class="small"><strong>Suivi :</strong> ' + esc(o.shippingTracking || "—") + '</span> ' +
    ...
    A.qs("#mkOrdersBody").querySelectorAll("select[data-order]").forEach(function (sel) {
      ...
      sel.onchange = function () {
        A.adminFetch("/api/admin/marketplace/orders/" + encodeURIComponent(sel.dataset.order) + "/status", { method: "PUT", body: JSON.stringify({ status: sel.value }) }).then(function () { location.reload(); }).catch(function (e) { alert(e.message || "Mise à jour impossible"); });
      };
    });
