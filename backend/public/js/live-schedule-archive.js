(function(){
  ...
  function renderArchive(a){
    ...
    var buyerRows=(a.buyers||[]).map(function(b){var items=(b.paidPurchases||[]).map(function(p){return esc(p.productName)+(p.spotLabel?" — "+esc(p.spotLabel):"")+" × "+Number(p.qty||1)+" ("+euro(p.amount)+")";}).join("<br>")||"Aucun achat payé",ad=b.shippingAddress||{},address=[ad.recipientName,ad.addressLine1,ad.addressLine2,[ad.postalCode,ad.city].filter(Boolean).join(" "),ad.countryCode].filter(Boolean).map(esc).join("<br>")||"Adresse manquante";return "<tr><td><strong>"+esc(b.pseudo||"Client")+"</strong><br><small>"+esc(b.customerEmail||"")+"</small></td><td>"+address+"</td><td>"+items+"</td><td>"+Math.round(Number(b.totalWeightGrams||0))+" g</td><td>"+esc(b.shippingCarrier||"—")+"<br><small>"+esc(b.shippingPackId||"")+"</small></td><td>"+euro(b.totalShippingPaid||0)+"</td><td>"+euro(b.totalPaid||0)+"</td></tr>";}).join("");
    box.innerHTML="<div class='live-archive-title'><div><h3>"+esc(a.title||a.liveId)+"</h3><p>Début : "+esc(formatDate(a.startedAt))+" · Fin : "+esc(formatDate(a.endedAt))+"</p></div><button type='button' id='liveArchiveCsv' class='btn'>Exporter CSV envois</button></div><div class='live-archive-summary'><strong>"+Number(a.totals&&a.totals.buyers||0)+" acheteur(s)</strong><span>"+Number(a.totals&&a.totals.paidPurchases||0)+" achat(s) payé(s)</span><span>Total : "+euro(a.totals&&a.totals.paidAmount||0)+"</span><span>Port encaissé : "+euro(a.totals&&a.totals.shippingAmount||0)+"</span></div><div class='live-archive-table-wrap'><table><thead><tr><th>Pseudo / client</th><th>Adresse postale</th><th>Achats</th><th>Poids</th><th>Envoi prévu</th><th>Port payé</th><th>Total payé</th></tr></thead><tbody>"+(buyerRows||"<tr><td colspan='7'>Aucun achat payé.</td></tr>")+"</tbody></table></div><p class='live-archive-note'>L'archive conserve aussi les paiements en attente/refusés pour contrôle, mais le tableau d'envoi affiche les achats payés.</p>";
    document.getElementById("liveArchiveCsv").onclick=function(){exportCsv(a);};
  }
  function exportCsv(a){
    var rows=[["Pseudo","Email","Destinataire","Adresse","Complement","Code postal","Ville","Pays","Telephone","Articles","Poids g","Transporteur","Pack","Port paye","Total paye"]];
    (a.buyers||[]).filter(function(b){return(b.paidPurchases||[]).length;}).forEach(function(b){var items=(b.paidPurchases||[]).map(function(p){return p.productName+(p.spotLabel?" - "+p.spotLabel:"")+" x"+(p.qty||1);}).join(" | ");var ad=b.shippingAddress||{};rows.push([b.pseudo||"",b.customerEmail||"",ad.recipientName||"",ad.addressLine1||"",ad.addressLine2||"",ad.postalCode||"",ad.city||"",ad.countryCode||"",ad.phone||"",items,b.totalWeightGrams||0,b.shippingCarrier||"",b.shippingPackId||"",b.totalShippingPaid||0,b.totalPaid||0]);});
