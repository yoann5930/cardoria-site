...
function liveShippingPaymentPolicy({live,email,itemAmount,shipping,coverage,actionState}){
  const active=new Set(["pending","paid","completed","authorized","authorised"]);
  const prior=listLiveCheckouts({liveId:live.id}).filter((checkout)=>String(checkout.customerEmail||"").trim().toLowerCase()===email&&active.has(String(checkout.status||"").toLowerCase()));
  ...
  return{
    buyerAmount:money(shipping.totalShippingTarget??shipping.shippingAmount),
    payer:"buyer",
    giveawayWinner,
    qualifyingGiveawayPurchase,
    locked:true
  };
}
