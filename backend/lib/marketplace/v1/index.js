...
export function getMarketplaceStats() {
  ...
  return {
    ...
    listingsActive: db.prepare("SELECT COUNT(*) AS c FROM mk_listings WHERE status = 'active' AND COALESCE(moderation_locked,0)=0").get()?.c ?? 0,
    ...
    disputesOpen: db.prepare("SELECT COUNT(*) AS c FROM mk_disputes WHERE status NOT IN ('resolved','rejected','closed')").get()?.c ?? 0,
