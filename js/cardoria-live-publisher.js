(function (global) {
  ...
  async function publish(opts) {
    ...
    if(!response.ok||payload.ok===false){stream.getTracks().forEach(function(track){track.stop();});bootstrap.close();var startError=new Error(payload.error||"Publication WebRTC impossible.");startError.status=response.status;throw startError;}
    ...
    bootstrap.close(); var publisherKey=payload.publisherKey, peers=new Map(), stopped=false,pollDelay=2500,pollTimer=null,pollBusy=false;
    ...
    async function poll(){if(stopped||pollBusy)return;pollBusy=true;try{var data=await post("/api/live/webrtc/publisher/offers",{liveSessionId:liveId,sourceId:sourceId,publisherKey:publisherKey},headers);pollDelay=2500;(data.offers||[]).forEach(function(item){void handleOffer(item);});}catch(e){if(e&&e.status===429)pollDelay=Math.min(10000,pollDelay+1500);}finally{pollBusy=false;schedulePoll(pollDelay);}}
    schedulePoll(0);
