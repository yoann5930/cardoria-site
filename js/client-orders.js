...
function render(orders){
  ...
  list.innerHTML=orders.map(o=>{
    ...
    const follow=url?'<a class="client-auth-primary client-order-link" href="'+esc(url)+'" target="_blank" rel="noopener noreferrer">Suivre mon colis →</a>':'';
    const shipping='<div class="client-order-shipping"><div><small>TRANSPORTEUR</small><strong>'+esc(o.carrier||o.shipping||"Non renseigné")+'</strong></div><div><small>FRAIS DE PORT</small><strong>'+euro(o.shippingCost)+'</strong></div>'+(o.tracking?'<div><small>NUMÉRO DE SUIVI</small><strong>'+esc(o.tracking)+'</strong></div>':'')+follow+label+'</div>'+(o.tracking?fallback:'<p class="client-order-wait">Le numéro de suivi apparaîtra ici dès que votre colis sera expédié.</p>');
    return '<article class="client-order-card"><div class="client-order-head"><div><small>COMMANDE BOUTIQUE</small><h2>'+esc(o.id)+'</h2><p>'+esc(o.date||"")+' · Paiement : '+esc(paymentLabel(o.paymentStatus))+'</p></div><strong class="client-order-total">'+euro(o.total)+'</strong></div>'+steps(o.status)+'<ul class="client-order-items">'+items+'</ul>'+shipping+'</article>';
  }).join("");
