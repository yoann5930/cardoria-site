(function(){
  ...
  function render(list){A.qs("#scBody").innerHTML=(list||[]).map(function(x){var product=(x.imageUrl?'<img src="'+esc(x.imageUrl)+'" alt="" style="width:42px;height:58px;object-fit:contain;vertical-align:middle;margin-right:8px" onerror="this.style.display=\'none\'">':'')+'<strong>'+esc(x.name)+'</strong>';return '<tr><td>'+product+'</td><td>'+esc(x.extension||"—")+'</td><td>'+esc(label(x.packaging))+'</td><td>'+esc(x.unitsPerPackage||1)+'</td><td>'+esc(x.ean||"—")+'</td><td>'+esc(euro(x.marketPrice))+'</td><td><strong>'+esc(euro(x.salePrice))+'</strong>'+(x.salePriceManual?' <span class="admin-badge">manuel</span>':'')+'</td><td>'+esc(x.priceSource||x.source||"—")+'</td><td><button class="btn btn-secondary scEdit" data-id="'+esc(x.id)+'">Modifier</button> <button class="btn btn-secondary scDelete" data-id="'+esc(x.id)+'">Supprimer</button></td></tr>';}).join("")||'<tr><td colspan="9">Aucun produit scellé en base. Synchronisation automatique en cours…</td></tr>';A.qs("#scCount").textContent=(list||[]).length+" référence(s) scellée(s) affichée(s)";A.qs("#scBody").querySelectorAll(".scEdit").forEach(function(b){b.onclick=function(){var x=current.find(function(i){return i.id===b.dataset.id;});if(x)fill(x);};});A.qs("#scBody").querySelectorAll(".scDelete").forEach(function(b){b.onclick=async function(){if(!confirm("Supprimer cette référence scellée ?"))return;var d=await A.adminFetch(API+"/"+encodeURIComponent(b.dataset.id),{method:"DELETE"});if(!d.ok){A.qs("#scMsg").textContent=d.error||"Suppression impossible.";return;}load();};});}
