...
router.put("/orders/:id/status", (req, res) => {
  try { const order = updateOrderStatus(req.params.id, req.body.status, req.body); logAudit({ type: "marketplace", action: "order_status", user: "admin", detail: `${req.params.id} → ${req.body.status}` }); res.json({ ok: true, order }); }
...
router.put("/sellers/:id/verified", (req, res) => { const seller = setSellerVerified(req.params.id, req.body.verified); logAudit({ type: "marketplace", action: "seller_verified", user: "admin", detail: req.params.id }); res.json({ ok: true, seller }); });
...
router.get("/config", (req, res) => {
  ...
  res.json({ ok: true, boutique: { provider: "sumup", configured: isSumUpConfigured() }, marketplace: { provider: "paypal", configured: paypal.configured, environment: paypal.environment, commissionPercent: paypal.commissionPercent, delayedDisbursement: paypal.delayedDisbursement }, carriers: ["mondial_relay", "colissimo", "chronopost"], carrierLabelsReady: false, stats: getMarketplaceStats() });
});
router.get("/stats", (req, res) => res.json({ ok: true, stats: getMarketplaceStats() }));
router.put("/orders/:id/tracking", (req, res) => {
  try { const order = updateOrderStatus(req.params.id, req.body.status || "shipped", { tracking: String(req.body.tracking || "").slice(0, 120), labelUrl: String(req.body.labelUrl || "").slice(0, 1000) }); logAudit({ type: "marketplace", action: "tracking", user: "admin", detail: req.params.id }); res.json({ ok: true, order }); }
  catch (e) { res.status(400).json({ ok: false, error: e.message }); }
});

// Ne jamais afficher "rembourse" sans appel reel au prestataire de paiement.
router.post("/orders/:id/refund", (req, res) => {
  res.status(501).json({ ok: false, error: "Remboursement automatique non active. Effectuez le remboursement dans PayPal/SumUp, puis synchronisez le statut via le webhook officiel." });
});
...
router.put("/disputes/:id", (req, res) => { const dispute = resolveDispute(req.params.id, req.body || {}); logAudit({ type: "marketplace", action: "dispute_resolve", user: "admin", detail: req.params.id }); res.json({ ok: true, dispute }); });
