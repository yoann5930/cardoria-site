...
export async function initMarketplacePersistence() {
  ...
  try {
    ...
    if (restored.restored) { console.log("[cardoria-persistence] marketplace + runtime restored from PostgreSQL"); return { ok: true, configured: true, restored: true, lastSyncedAt: restored.lastSyncedAt }; }
    const seeded = await snapshotNow("initial-sqlite-export");
    console.log("[cardoria-persistence] PostgreSQL initialized from runtime");
...
export async function closeMarketplacePersistence() { if (!pool) return; const current = pool; pool = null; await current.end(); }
