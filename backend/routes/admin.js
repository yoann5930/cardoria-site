...
import { applyStockPrefsToNotes } from "../lib/boutique/stock.js";
...
router.put("/stock/preferences/:id",WRITE_ADMIN,async(req,res)=>{try{const result=await withBoutiqueOrderLock(()=>{const purchases=readJson("purchases",DEFAULT_PURCHASES),i=purchases.findIndex((p)=>String(p.id)===String(req.params.id));if(i<0)return{missing:true};if(typeof req.body?.notes!=="string")throw Object.assign(new Error("Préférences de stock invalides."),{status:400});const notes=applyStockPrefsToNotes(purchases[i].notes,cleanText(req.body.notes,10000));const purchase={...purchases[i],notes,updatedAt:new Date().toISOString()};purchases[i]=purchase;writeJson("purchases",purchases);return{purchase};});if(result.missing)return res.status(404).json({ok:false,error:"Achat source introuvable."});logAudit({type:"stock",action:"stock_preferences_update",user:req.authUser?.email||"admin",detail:result.purchase.id});res.json({ok:true,purchase:result.purchase});}catch(e){res.status(e.status||400).json({ok:false,error:e.message||"Enregistrement du stock impossible."});}});
