...
function base32Decode(input) {
  const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567"; const str = String(input || "").toUpperCase().replace(/=+$/, ""); let bits = "";
...
async function loginSuperAdmin() {
  ...
  assert(first.response.status === 200 && first.body.ok, "Admin login failed");
  if (first.body.token && first.body.requires2fa !== true) return { token: first.body.token, actor: first.body.user };
  assert(first.body.requires2fa === true, "Admin login returned no valid authentication flow");
  const secret = first.body.setup?.secret || "";
  assert(secret, "2FA is enabled but CI cannot complete an existing challenge without a test secret");
...
let sellerBypassBlocked = false; try { updateListingV1(listing.id, seller.id, { status: "active" }); } catch (error) { sellerBypassBlocked = error?.status === 409; }
