(function () {
  ...
  function buyerLabel(v){return String(v||"yoann").toLowerCase()==="valentin"?"Valentin":"Yoann";}
  ...
  function loadAll() {
    ...
    A.adminFetch("/api/admin/accounting/stats").then(function (d) {
      ...
      A.qs("#accountingYoannTotal").textContent=A.euro(d.purchaseByBuyer&&d.purchaseByBuyer.yoann||0); A.qs("#accountingValentinTotal").textContent=A.euro(d.purchaseByBuyer&&d.purchaseByBuyer.valentin||0);
    });
  }
  ...
  A.renderShell("accounting", "Comptabilité", "Historique, achats, acheteurs, exports et statistiques financières",
    ...
    '<div class="admin-filters"><input id="searchQ" placeholder="Recherche multicritères..."><select id="filterLicense"><option value="">Toutes licences</option><option value="pokemon">Pokémon</option><option value="yugioh">Yu-Gi-Oh!</option><option value="onepiece">One Piece</option><option value="lorcana">Lorcana</option><option value="magic">Magic</option></select><select id="filterBuyer"><option value="">Tous les acheteurs</option><option value="yoann">Yoann</option><option value="valentin">Valentin</option></select><button class="btn btn-primary" type="button" id="expCsvSales">Export Excel (CSV)</button><button class="btn btn-secondary" type="button" id="expPdfSales">Export imprimable</button><button class="btn btn-secondary" type="button" id="expCsvPurch">Export achats CSV</button></div>' +
