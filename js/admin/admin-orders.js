(function () {
  ...
  function renderOrders() {
    ...
    var list = orders.filter(function (o) { var ok = filter === "all" || (filter === "today" ? o.date === todayStr() : o.status === filter); var txt = String((o.id || "") + (o.client || "") + (o.email || "") + (o.status || "") + (o.tracking || "")).toLowerCase(); return ok && txt.includes(q); });
    A.qs("#orderCards").innerHTML = list.map(function (o) {
      return '<article class="request-card" style="margin-bottom:16px"><div class="request-head"><div><h3>' + esc(o.id) + "</h3><p>" + esc(o.date) + " • " + esc(o.client) + "<br>" + esc(o.email) + '</p></div><strong>' + euro(orderTotal(o)) + "</strong></div><div class="progress">" + statusSteps(o.status) + "</div><p><b>Livraison :</b> " + esc(o.shipping || "À définir") + " • <b>Suivi :</b> " + esc(o.tracking || "Non renseigné") + " • <b>Paiement :</b> " + esc(o.payment || o.paymentStatus || "—") + "</p><div class='actions'><button type='button' class='btn btn-secondary' data-doc='" + esc(o.id) + "' data-type='bon'>Bon commande</button><button type='button' class='btn btn-secondary' data-doc='" + esc(o.id) + "' data-type='facture'>Facture</button></div></article>";
  ...
  A.renderShell("orders", "Commandes", "Suivi des ventes Boutique en temps réel", '<div class="admin-filters"><input id="orderSearch" placeholder="Rechercher commande..." oninput="renderOrdersList()"><button class="btn btn-secondary" type="button" data-filter="all">Toutes</button><button class="btn btn-secondary" type="button" data-filter="today">Aujourd\'hui</button><button class="btn btn-secondary" type="button" data-filter="À préparer">À préparer</button></div><div id="orderCards"></div>');
