(function () {
  ...
  function statusLabel(v) { return v === "À préparer" ? "Commande confirmée" : v; }
  function options(values, current, emptyLabel, statusMode) { var out = emptyLabel ? '<option value="">'+esc(emptyLabel)+'</option>' : ""; return out + values.map(function (v) { return '<option value="'+esc(v)+'"'+(v===current?' selected':'')+'>'+esc(statusMode ? statusLabel(v) : v)+'</option>'; }).join(""); }
  ...
  function statusSteps(current) {
    ...
    return steps.map(function (s, i) { return '<span class="'+(i<=idx?"active":"")+'">'+esc(statusLabel(s))+'</span>'; }).join("");
  }
    ...
    A.qs("#orderCards").innerHTML = list.map(function (o) {
      ...
      return '<article class="request-card" data-order-card="'+esc(o.id)+'" style="margin-bottom:18px">' +
        ...
        '<div class="admin-form-grid" style="margin-top:14px">' +
          '<label>État de la commande<select data-field="status">'+options(STATUSES,o.status,"",true)+'</select></label>' +
          '<label>Transporteur<select data-field="carrier">'+options(legacyCarrier,o.carrier||"","Choisir",false)+'</select></label>' +
  ...
  A.renderShell("orders","Commandes Boutique","Gestion des commandes, préparation, expédition et livraison",
    '<div class="admin-kpi-grid" style="margin-bottom:16px"><div class="admin-kpi"><label>Total commandes</label><strong id="ordersCount">—</strong></div><div class="admin-kpi"><label>Commandes confirmées</label><strong id="ordersPrepare">—</strong></div><div class="admin-kpi"><label>Expédiées</label><strong id="ordersShipped">—</strong></div><div class="admin-kpi"><label>CA payé</label><strong id="ordersRevenue">—</strong></div></div>' +
    '<div class="admin-filters"><input id="orderSearch" placeholder="Commande, client, email, suivi..."><button class="btn btn-secondary" data-filter="all">Toutes</button><button class="btn btn-secondary" data-filter="today">Aujourd\'hui</button><button class="btn btn-secondary" data-filter="À préparer">Confirmées</button><button class="btn btn-secondary" data-filter="En préparation">En préparation</button><button class="btn btn-secondary" data-filter="Expédiée">Expédiées</button><button class="btn btn-secondary" data-filter="Livrée">Livrées</button><button class="btn btn-secondary" data-filter="Annulée">Annulées</button></div><div id="orderCards"></div>');
