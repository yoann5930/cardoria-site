...
function normalizeServicePoint(raw){
  ...
  return{
    id,
    carrierServicePointId:clean(raw.carrierServicePointId,120),
    name:clean(raw.name,160),
    address:clean(raw.address,200),
    postalCode:clean(raw.postalCode,24).toUpperCase(),
    city:clean(raw.city,120),
    countryCode:clean(raw.countryCode||"FR",2).toUpperCase(),
    carrierCode:clean(raw.carrierCode||"mondial_relay",80)
  };
}
function priorServicePoint(liveId,email){
  const usable=new Set(["planned","pending","paid","completed","authorized","authorised"]);
  const prior=listLiveCheckouts({liveId}).find((c)=>String(c.customerEmail||"").toLowerCase()===email&&usable.has(String(c.status||"").toLowerCase())&&c.servicePoint?.id);
  return prior?.servicePoint||null;
}
...
function liveShippingPaymentPolicy({live,email,itemAmount,shipping,coverage,actionState}){
  // A pending/authorized payment is not evidence that postage was actually paid.
  const settled=new Set(["paid","completed"]);
  const prior=listLiveCheckouts({liveId:live.id}).filter((checkout)=>String(checkout.customerEmail||"").trim().toLowerCase()===email&&settled.has(String(checkout.status||"").toLowerCase()));
  const priorBuyerPaid=prior.some((checkout)=>checkout.shippingPaidBy==="buyer"&&Number(checkout.shippingAmount||0)>0);
  const priorCardoriaCovered=prior.some((checkout)=>checkout.shippingCoveredBySellerPlan===true);
  const winnerEmail=String(actionState?.giveaway?.winner?.email||"").trim().toLowerCase();
  const giveawayWinner=Boolean(winnerEmail&&winnerEmail===email);
  const qualifyingGiveawayPurchase=Number(itemAmount||0)>=10;
  if(coverage.covered||priorCardoriaCovered){
    return{buyerAmount:0,payer:"cardoria",giveawayWinner,qualifyingGiveawayPurchase,locked:true};
  }
  if(priorBuyerPaid){
    return{buyerAmount:0,payer:"buyer",giveawayWinner,qualifyingGiveawayPurchase,locked:true};
  }
  if(giveawayWinner&&!qualifyingGiveawayPurchase){
    return{buyerAmount:0,payer:"streamer",giveawayWinner,qualifyingGiveawayPurchase,locked:false};
  }
  return{buyerAmount:money(shipping.totalShippingTarget??shipping.shippingAmount),payer:"buyer",giveawayWinner,qualifyingGiveawayPurchase,locked:true};
}
...
export async function createLiveCheckout(input){
  ...
  return runLiveCheckoutTask("shipping-live:"+String(input?.liveId||""),async()=>{
    ...
    if(current.status==="paid")throw Object.assign(new Error("Ce paiement Live a déjà été traité."),{status:409});
    ...
    if(current.provider==="sumup"){
      ...
      const payment=await createSumUpCheckout({
        orderId:current.id,amount:current.amount,
        description:`Live Cardoria — ${current.productName}${current.spotLabel?` — ${current.spotLabel}`:""}${current.shippingAmount?` — port ${current.shippingAmount.toFixed(2)} EUR`:""}`,
        customerEmail:current.customerEmail,redirectUrl:String(input.successUrl||"/live.html"),source:"live_cardoria"
      });
      current.status="pending";current.paymentProviderOrderId=payment.checkoutId;current.url=payment.url;current.environment="production";current.updatedAt=new Date().toISOString();
    ...
    const payment=await createLivePayPalOrder({
      checkoutId:current.id,amount:current.amount,sellerId:current.ownerId,
      description:`Live vendeur Cardoria — ${current.productName}${current.spotLabel?` — ${current.spotLabel}`:""}${current.shippingAmount?` — port ${current.shippingAmount.toFixed(2)} EUR`:""}`,
      successUrl:String(input.successUrl||"/live.html"),cancelUrl:String(input.cancelUrl||"/live.html"),platformFee:current.platformFee
    });
    current.status="pending";current.paymentProviderOrderId=payment.id;current.url=payment.url;current.environment=paypal.environment;current.updatedAt=new Date().toISOString();
    return saveLiveCheckout(current);
  });
}
