...
function fail(res, error, fallback = 400) {
  return res.status(error?.status || error?.code || fallback).json({
    ok: false,
    error: error?.message || "Erreur Live",
    provider: error?.provider || error?.expectedProvider,
    expectedProvider: error?.expectedProvider,
    requestedProvider: error?.requestedProvider
  });
}
...
router.get("/sessions", (req, res) => {
  ...
  const sessions = listLiveSessions({ status: status === "all" ? undefined : status }).map(publicLiveSession);
