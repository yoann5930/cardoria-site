(function () {
  ...
  function clearAdminSession() {
    ...
    sessionStorage.removeItem("cardoria_session_expires_at");
  }
  ...
  function validateAdminSession() {
    ...
    return fetch(BACKEND + "/api/auth/me", {
      ...
      // Une panne réseau transitoire ne doit pas déconnecter l'administrateur.
      return true;
    }).finally(function () {
  ...
  function protectAdmin() {
    ...
    if (!sessionValidationStarted) {
      ...
      setTimeout(validateAdminSession, 20);
      setInterval(validateAdminSession, 120000);
    }
    return true;
  }
    ...
    function attempt(index) {
      ...
      return fetch(BACKEND + path, requestOptions).then(function (response) {
        if (response.status === 401 || response.status === 403) {
          clearAdminSession();
          goToLogin();
          return { ok: false, error: "Session expirée.", status: response.status };
        }
        ...
        return response.text().then(function (text) {
          ...
          if (!response.ok && !data.error) data.error = "Erreur serveur (" + response.status + ").";
  ...
  function itemActive(item, activePage) {
    ...
    return Array.isArray(item.children) && item.children.some(function (child) { return child.page === activePage; });
  }

  function renderNavItem(item, activePage) {
    if (Array.isArray(item.children) && item.children.length) {
      var open = item.children.some(function (child) { return child.page === activePage; });
      var children = item.children.map(function (child) {
        var cls = child.page === activePage ? " active" : "";
        return '<a class="admin-subnav-link' + cls + '" href="' + child.href + '" style="padding-left:34px"><span class="admin-subnav-dot">↳</span><span>' + child.label + '</span></a>';
      }).join("");
      return '<div class="admin-purchase-menu' + (open ? ' is-open' : '') + '">' +
        '<button type="button" class="admin-subnav-link admin-purchase-menu-btn" style="width:100%;border:0;background:transparent;text-align:left" aria-expanded="' + (open ? 'true' : 'false') + '"><span class="admin-subnav-dot">•</span><span>' + item.label + '</span><span style="margin-left:auto">›</span></button>' +
        '<div class="admin-purchase-children" style="display:' + (open ? 'block' : 'none') + '">' + children + '</div></div>';
    }
  ...
  function renderShell(activePage, title, subtitle, mainHtml) {
    var navHtml = NAV_GROUPS.map(function (group, groupIndex) {
      var groupActive = group.items.some(function (item) { return itemActive(item, activePage); });
      var links = group.items.map(function (item) { return renderNavItem(item, activePage); }).join("");
    ...
    document.querySelectorAll("[data-admin-nav-group]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        if (nextOpen) { group.classList.add("is-open"); button.setAttribute("aria-expanded", "true"); }
      });
    });

    document.querySelectorAll(".admin-purchase-menu-btn").forEach(function (button) {
      button.addEventListener("click", function () {
        var menu = button.closest(".admin-purchase-menu");
        var box = menu && menu.querySelector(".admin-purchase-children");
        if (!menu || !box) return;
  ...
  function formatMarketIndex(idx, rec) {
    ...
    if (rec && rec.primary) {
      ...
      if (rec.resale && rec.resale.label !== rec.primary.label) recHtml += "<tr><th>Stratégie revente</th><td><span class='admin-badge'>" + rec.resale.label + "</span><br><span style='font-size:12px;color:#baaf97'>" + (rec.resale.detail || "") + "</span></td></tr>";
    }
    return "<div class='admin-panel' style='padding:12px;margin-top:10px;border:1px solid rgba(212,175,55,.25)'><h3 style='margin:0 0 10px;color:#ffe18a;font-size:16px'>Cardoria Market Score : <strong style='font-size:22px'>" + idx.cardoriaMarketScore + "</strong>/100</h3><table class='admin-table' style='font-size:13px'><tr><th>Demande actuelle</th><td>" + (idx.demand ?? "—") + " / 100</td></tr><tr><th>Rareté réelle</th><td>" + (idx.rarity?.label || "—") + (idx.rarity?.raw ? " (" + idx.rarity.raw + ")" : "") + "</td></tr><tr><th>Vitesse de vente</th><td>" + (idx.salesVelocity?.label || "—") + " (" + (idx.salesVelocity?.perWeek ?? 0) + "/sem.)</td></tr><tr><th>Ventes récentes (30 j)</th><td>" + (idx.recentSalesCount ?? 0) + "</td></tr><tr><th>Évolution 7 j</th><td>" + fmtEv(ev.days7) + "</td></tr><tr><th>Évolution 30 j</th><td>" + fmtEv(ev.days30) + "</td></tr><tr><th>Évolution 90 j</th><td>" + fmtEv(ev.days90) + "</td></tr><tr><th>Tendance</th><td>" + (idx.trend?.label || "—") + "</td></tr><tr><th>Liquidité</th><td>" + (idx.liquidity?.label || "—") + " (score " + (idx.liquidity?.score ?? "—") + ")</td></tr><tr><th>Niveau collection</th><td>" + (idx.collectionLevel?.label || "—") + "</td></tr><tr><th>Popularité licence</th><td>" + (idx.popularity?.score ?? "—") + " / 100</td></tr>" + recHtml + "</table></div>";
  }

  function formatAdminPricing(p) {
    if (!p) return "<p>—</p>";
    var m = p.market || p;
    var rot = p.estimatedRotationLabel ? "<tr><th>Rotation estimée</th><td>" + p.estimatedRotationLabel + (p.estimatedRotationDays != null ? " (" + p.estimatedRotationDays + " j)" : "") + "</td></tr>" : "";
    return "<div class='admin-panel' style='padding:12px;margin-top:10px'><h3 style='margin:0 0 10px;color:#ffe18a;font-size:16px'>Estimation achat / revente</h3><table class='admin-table' style='font-size:13px'><tr><th>Marché bas</th><td>" + euro(m.low ?? p.low) + "</td></tr><tr><th>Marché moyen</th><td>" + euro(m.avg ?? p.avg) + "</td></tr><tr><th>Marché haut</th><td>" + euro(m.high ?? p.high) + "</td></tr><tr><th>Rachat Cardoria</th><td>" + (p.buyback != null ? euro(p.buyback) : "<span class='admin-badge admin-badge--danger'>À vérifier en main</span>") + "</td></tr><tr><th>Revente conseillée</th><td>" + euro(p.resell ?? p.recommended) + "</td></tr><tr><th>Marge estimée</th><td>" + (p.margin != null ? euro(p.margin) + " (" + (p.marginPercent || "—") + " %)" : "—") + "</td></tr><tr><th>Marge cible</th><td>" + (p.targetMarginRate != null ? p.targetMarginRate + " %" : "—") + "</td></tr>" + rot + "<tr><th>Liquidité</th><td>" + (p.liquidityLabel || "—") + "</td></tr><tr><th>Niveau confiance</th><td>" + (p.confidenceLevel || "—") + (p.confidenceScore != null ? " — " + p.confidenceScore + " %" : "") + "</td></tr>" + (p.adminNote ? "<tr><th>Note</th><td>" + p.adminNote + "</td></tr>" : "") + "</table></div>" + formatCardoriaIntelligence(p.intelligence) + formatMarketIndex(p.marketIndex, p.adminRecommendation);
  }

  function formatCardoriaIntelligence(intel) {
    if (!intel) return "";
    var s = intel.scores || {}, p = intel.pricing || {}, f = intel.forecasts || {}, h = intel.history || {}, rec = intel.recommendation || {};
    function histRow(key, label) { var pts = h[key]?.points?.length || 0; return "<tr><th>Historique " + label + "</th><td>" + pts + " point(s)</td></tr>"; }
    return "<div class='admin-panel' style='padding:12px;margin-top:10px;border:1px solid rgba(212,175,55,.35)'><h3 style='margin:0 0 10px;color:#ffe18a;font-size:16px'>Intelligence Cardoria Premium — Score global : <strong style='font-size:22px'>" + (s.overall ?? "—") + "</strong>/100</h3><table class='admin-table' style='font-size:13px'><tr><th colspan='2' style='color:#ffe18a'>Score Cardoria (composantes)</th></tr>" + (s.authenticity != null ? "<tr><th>Authenticité</th><td>" + s.authenticity + " %</td></tr>" : "") + "<tr><th>État</th><td>" + (s.condition ?? "—") + " %</td></tr><tr><th>Liquidité</th><td>" + (s.liquidity ?? "—") + " %</td></tr><tr><th>Popularité</th><td>" + (s.popularity ?? "—") + " %</td></tr><tr><th>Rareté</th><td>" + (s.rarity ?? "—") + " %</td></tr><tr><th>Potentiel futur</th><td>" + (s.futurePotential ?? "—") + " %</td></tr><tr><th colspan='2' style='color:#ffe18a'>Prix marché</th></tr><tr><th>Moyen</th><td>" + euro(p.marketAverage ?? intel.market?.average) + "</td></tr><tr><th>Minimum actuel</th><td>" + euro(p.marketMinimum ?? intel.market?.minimum) + "</td></tr><tr><th>Maximum actuel</th><td>" + euro(p.marketMaximum ?? intel.market?.maximum) + "</td></tr><tr><th>Prix conseillé Cardoria</th><td>" + euro(p.cardoriaRecommended) + "</td></tr><tr><th>Vente rapide</th><td>" + euro(p.quickSale) + "</td></tr><tr><th>Vente optimale</th><td>" + euro(p.optimalSale) + "</td></tr><tr><th>Achat professionnel</th><td>" + (p.professionalBuy != null ? euro(p.professionalBuy) : "—") + "</td></tr><tr><th>Marge potentielle</th><td>" + (intel.margin?.amount != null ? euro(intel.margin.amount) + " (" + (intel.margin.percent ?? "—") + " %)" : "—") + "</td></tr><tr><th>Rentabilité estimée</th><td>" + (intel.margin?.estimatedProfitability != null ? intel.margin.estimatedProfitability + " % / an (proxy)" : "—") + "</td></tr><tr><th colspan='2' style='color:#ffe18a'>Indices & tendance</th></tr><tr><th>Indice rareté</th><td>" + (intel.indices?.rarity?.score ?? "—") + " — " + (intel.indices?.rarity?.label || "") + "</td></tr><tr><th>Indice liquidité</th><td>" + (intel.indices?.liquidity?.score ?? "—") + " — " + (intel.indices?.liquidity?.label || "") + "</td></tr><tr><th>Tendance</th><td>" + (intel.trend?.label || "—") + " (" + (intel.trend?.direction || "stable") + ")</td></tr>" + histRow("7d", "7 j") + histRow("30d", "30 j") + histRow("90d", "90 j") + histRow("1y", "1 an") + "<tr><th colspan='2' style='color:#ffe18a'>Prévisions IA</th></tr><tr><th>30 jours</th><td>" + euro(f.days30?.price) + " (" + (f.days30?.changePercent ?? 0) + " %, conf. " + (f.days30?.confidence ?? "—") + " %)</td></tr><tr><th>90 jours</th><td>" + euro(f.days90?.price) + " (" + (f.days90?.changePercent ?? 0) + " %, conf. " + (f.days90?.confidence ?? "—") + " %)</td></tr><tr><th>1 an</th><td>" + euro(f.days365?.price) + " (" + (f.days365?.changePercent ?? 0) + " %, conf. " + (f.days365?.confidence ?? "—") + " %)</td></tr><tr><th>Recommandation client</th><td><span class='admin-badge admin-badge--gold'>" + (rec.label || "—") + "</span><br><span style='font-size:12px;color:#baaf97'>" + (rec.detail || rec.clientHint || "") + "</span></td></tr>" + (intel.marketStats ? formatMarketStatsBlock(intel.marketStats) : "") + "</table></div>";
  }
