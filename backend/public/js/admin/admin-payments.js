(function () {
  ...
  function renderList() {
    ...
    A.qs("#payList").innerHTML = rows.map(function (p) {
      ...
      return "<tr>" +
        ...
        "<td><strong>" + A.euro(p.amount) + "</strong><br><small>" + esc(p.paymentMethod || "sumup_card") + "</small></td>" +
        ...
        "<td><small>Checkout</small><br>" + esc(p.sumupCheckoutId || "—") + "<br><small>Transaction</small><br>" + esc(p.sumupTransactionId || "—") + "</td>" +
  ...
  function renderSummary(summary, configured) {
    summary = summary || {};
    A.qs("#sumupConfigured").textContent = configured ? "ACTIF" : "NON CONFIGURÉ";
    A.qs("#sumupConfigured").className = "admin-badge " + (configured ? "admin-badge--ok" : "admin-badge--danger");
  ...
  function load() {
    ...
    A.adminFetch("/api/admin/payments/" + query).then(function (d) {
      ...
      renderSummary(d.summary, d.configured);
      renderList();
    }).catch(function () { alert("Impossible de charger les paiements SumUp."); });
  }
  ...
  function refundPayment(id, maxAmount, btn) {
    ...
    A.adminFetch("/api/admin/payments/" + encodeURIComponent(id) + "/refund", { method: "POST", body: JSON.stringify(body) }).then(function (d) {
      ...
      alert("Demande de remboursement envoyée à SumUp. Statut : " + (d.status || "en cours"));
      load();
    }).catch(function (e) { alert(e.message || "Remboursement impossible"); if (btn) { btn.disabled = false; btn.textContent = "Rembourser"; } });
  }

  A.renderShell("payments", "Paiements SumUp", "Pilotage financier SumUp : rapprochement, synchronisation et remboursements",
    '<div class="admin-panel" style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap"><div><h2 style="margin:0 0 6px">État SumUp</h2><p style="margin:0;color:#baaf97">Les statuts affichés proviennent du ledger serveur et sont rapprochés avec les commandes.</p></div><span id="sumupConfigured" class="admin-badge">...</span></div>' +
    '<div class="admin-kpi-grid">' +
      '<div class="admin-kpi"><label>Encaissé SumUp</label><strong id="payPaidAmount">0,00 €</strong><small id="payPaidCount">0 paiement</small></div>' +
    ...
    '<div class="admin-filters">' +
      '<input id="paySearch" placeholder="Commande, client, checkout, transaction...">' +
      '<select id="paySource"><option value="">Toutes sources</option><option value="boutique">Boutique</option><option value="marketplace">Marketplace legacy SumUp</option></select>' +
    ...
    '<div class="admin-panel"><h2>Historique et rapprochement</h2><div class="admin-table-wrap"><table class="admin-table"><thead><tr>' +
      '<th>Paiement / commande</th><th>Source</th><th>Client</th><th>Montant</th><th>Statut</th><th>Rapprochement</th><th>Références SumUp</th><th>Date</th><th>Actions</th>' +
