...
router.post("/v1/paypal/sellers/:id/onboard", async (req, res) => {
  try {
    ...
    res.json({ ok: true, ...(await createSellerOnboarding({ sellerId: seller.id, returnUrl: `${base}/vendre.html?paypal=return&seller=${encodeURIComponent(seller.id)}` })) });
  } catch (error) { fail(res, error); }
});
...
router.post("/v1/paypal/checkout", async (req, res) => {
  ...
  try {
    ...
    for (const order of orders) { try { updateOrderStatus(order.id, "cancelled", { paymentStatus: "failed", paymentMethod: "paypal" }); } catch {} }
    fail(res, error);
  }
});
...
router.post("/v1/paypal/orders/:paypalOrderId/capture", async (req, res) => {
  try {
    ...
    res.json({ ok: true, ...(await captureMarketplacePayPalOrder(req.params.paypalOrderId)) });
  } catch (error) { fail(res, error); }
});
