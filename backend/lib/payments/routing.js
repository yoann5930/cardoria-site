...
export const RETIRED_PROVIDERS = Object.freeze(["revolut"]);
...
export function resolvePaymentRoute({ channel, ownerRole } = {}) {
  ...
  if (!provider) throw Object.assign(new Error("Canal de paiement Cardoria inconnu."), { status: 400 });
...
export function assertSaleProvider({ channel, ownerRole, requestedProvider } = {}) {
  ...
  if (RETIRED_PROVIDERS.includes(requested)) {
    const error = new Error("Revolut n'est plus utilisé sur Cardoria.");
    error.status = 410;
...
export function assertServerAmount(serverAmount, clientAmount) {
  ...
  if (client !== server) throw Object.assign(new Error("Le montant envoyé par le client ne correspond pas au montant serveur."), { status: 403 });
  return server;
}
